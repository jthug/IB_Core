package com.lianer.core.manager;
...
import com.lianer.core.stuff.ReplyCode;
...
//                    HLWalletManager.shared().saveWallet(context, hlWallet);
        ...
        return flowable
                .flatMap(s -> {
                    ...
                    if (HLWalletManager.shared().isWalletExist(hlWallet.getAddress())) {
                        return Flowable.error(new HLError(ReplyCode.walletExisted, new Throwable(context.getString(R.string.wallet_existed))));
                    }
                    HLWalletManager.shared().saveWallet(context, hlWallet);
    ...
    public ECKeyPair generateKeyPair(String mnemonics) {
        ...
        KLog.i("InitWalletManager","address:" + Constant.PREFIX_16 + address);
    ...
    public Flowable<HLWallet> importPrivateKey(Context context, String privateKey, String password) {
        ...
        return flowable.flatMap(s -> {
            ...
            HLWallet hlWallet = new HLWallet(walletFile);
            if (HLWalletManager.shared().isWalletExist(hlWallet.getAddress())) {
                return Flowable.error(new HLError(ReplyCode.walletExisted, new Throwable(context.getString(R.string.s_existed))));
            }
            HLWalletManager.shared().saveWallet(context, hlWallet);
            return Flowable.just(hlWallet);
        });
