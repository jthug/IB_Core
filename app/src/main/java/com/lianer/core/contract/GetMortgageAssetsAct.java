...
import com.lianer.core.databinding.ActivityGetMortgageAssetsBinding;
 ...
 * 取回抵押资产
 */
public class GetMortgageAssetsAct extends BaseActivity implements View.OnClickListener {

    ActivityGetMortgageAssetsBinding mBinding;
    ...
    protected void initViews() {
        mBinding = DataBindingUtil.setContentView(this, R.layout.activity_get_mortgage_assets);
        ...
        mBinding.btnGetAssets.setOnClickListener(this);
    ...
    protected void initData() {
        ...
        mBinding.mortgageAssetsAmount.setText(Convert.fromWei(mContractInfo.getMortgageAssetsAmount(), Convert.Unit.ETHER)+"");
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.contract_address:
                ...
                break;
            case R.id.btn_get_assets:

                if (!mPassword.isEmpty()) {
                    invokeRepayment();
                } else {
                    CenterDialog centerDialog = new CenterDialog(R.layout.dlg_input_wallet_pwd, GetMortgageAssetsAct.this);
    ...
    private void invokeRepayment() {
        try {
            Flowable.just(1)
                    .flatMap(s -> IBContractUtil.loadIBContract(TransferUtil.getWeb3j(), mCredentials, mContractInfo.getContractAddress()))
                    .subscribeOn(Schedulers.newThread())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(new HLSubscriber<IBContract>(GetMortgageAssetsAct.this, true) {
                        @Override
                        protected void success(IBContract contract) {
                            //设置GasPrice
                            contract.setGasPrice(mGasPrice);
//                            Flowable.just(1)
//                                    .flatMap(s -> IBContractUtil.applyForAssets(contract))
//                                    .subscribeOn(Schedulers.newThread())
//                                    .observeOn(AndroidSchedulers.mainThread())
//                                    .subscribe(new HLSubscriber<String>(GetMortgageAssetsAct.this, true) {
//                                        @Override
//                                        protected void success(String data) {
//                                            mTxHash = data;
//                                            KLog.w(mTxHash);
//                                            mContractInfo.setContractStatus(ContractStatus.CONTRACT_STSTUS_23);
...
//                                    });
            ...
            public void onSuccess(Object responses) {
                ...
                if (responses != null) {
                    ...
                    if (responses.toString().equals("1")) {
                        KLog.i("修改成功");
                        Intent intent = new Intent(GetMortgageAssetsAct.this, ContractStateActivity.class);
    ...
    private void gasWarnDlgShow() {
        new KnowDialog(new CenterDialog(R.layout.dlg_messge_warn, GetMortgageAssetsAct.this), null, getString(R.string.gas_info), getString(R.string.gas_description), Gravity.LEFT);
    }
    ...
    private void checkETHBalance(BigDecimal gas) {
        if (Double.valueOf(mETHBalance) < gas.doubleValue()) {
            ...
            mBinding.btnGetAssets.setBackgroundResource(R.drawable.gray_oval_btn);
            mBinding.btnGetAssets.setTextColor(getResources().getColor(R.color.clr_666666));
            mBinding.btnGetAssets.setEnabled(false);
        } else {
            mBinding.ethAvailableAmount.setTextColor(getResources().getColor(R.color.clr_059EFF));
            mBinding.btnGetAssets.setBackgroundResource(R.drawable.gradient_oval_btn);
            mBinding.btnGetAssets.setTextColor(getResources().getColor(R.color.clr_F5F5F5));
            mBinding.btnGetAssets.setEnabled(true);
        }
    }
}
