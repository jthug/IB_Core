...
import com.lianer.core.databinding.ActivityRepaymentBinding;
...
public class RepaymentActivity extends BaseActivity implements View.OnClickListener {
    ...
    ActivityRepaymentBinding mBinding;
    ...
    private BigDecimal mRepaymentAmount;
    private IBContract mContract;
    private SecurityConfigure configure;

    @Override
    protected void initViews() {
        mBinding = DataBindingUtil.setContentView(this, R.layout.activity_repayment);
        ...
        switch (v.getId()){
            ...
            case R.id.btn_pay:
                ...
                if(!mPassword.isEmpty()){
                    ...
                    CenterDialog centerDialog = new CenterDialog(R.layout.dlg_input_wallet_pwd, RepaymentActivity.this);
    ...
    private void invokeRepayment(){
        try {
            Flowable.just(1)
                    ...
                    .subscribe(new HLSubscriber<IBContract>(RepaymentActivity.this,true) {
                        @Override
                        protected void success(IBContract contract) {
                            try {
                                //设置GasPrice
                                contract.setGasPrice(mGasPrice);
                                BigInteger rePaymentWei = new BigInteger(IBContractUtil.getIBContractStatus(contract).getExpire());
//                                Flowable.just(1)
//                                        .flatMap(s -> IBContractUtil.sendRepayment(contract, rePaymentWei))
//                                        .subscribeOn(Schedulers.newThread())
//                                        .observeOn(AndroidSchedulers.mainThread())
//                                        .subscribe(new HLSubscriber<String>(RepaymentActivity.this, true) {
//                                            @Override
//                                            protected void success(String data) {
//                                                mTxHash = data;
//                                                KLog.w(mTxHash);
//                                                //状态
//                                                mContractInfo.setContractStatus(ContractStatus.CONTRACT_STSTUS_18);
//                                                //Hash
...
//                                        });
            ...
            public void onSuccess(Object responses) {
                ...
                if (responses != null) {
                    ...
                    if (responses.toString().equals("1")) {
                        KLog.i("修改成功");
                        Intent intent = new Intent(RepaymentActivity.this, ContractStateActivity.class);
    ...
    private void gasWarnDlgShow() {
        new KnowDialog(new CenterDialog(R.layout.dlg_messge_warn, RepaymentActivity.this), null, getString(R.string.gas_info), getString(R.string.gas_description), Gravity.LEFT);
            ...
            Flowable.just(1)
                    ...
                    .subscribe(new HLSubscriber<IBContract>(RepaymentActivity.this,true) {
                        ...
                        protected void success(IBContract contract) {
                            ...
                            //到期本息
                            try {
                                mRepaymentAmount =  new BigDecimal(IBContractUtil.getIBContractStatus(contract).getExpire());
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                            mBinding.duePayment.setText(mRepaymentAmount+" ETH");
                            getEthBanlance();
                        }
    ...
    private  void updateSeekBar(int progress){
        ...
        BigDecimal value = gas.add(mRepaymentAmount);
